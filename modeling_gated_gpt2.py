...
@dataclass
class ProbingViaPromptingOutputs(ModelOutput):
    """
...
class GPT2Block(nn.Module):
    ...
    def apply_masks(self, head_mask):
        self.attn.apply_masks(head_mask)
    
    def get_masks(self):
        return self.attn.get_masks()
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def apply_masks(self, head_mask):
        for layer, module in enumerate(self.h):
            module.apply_masks(head_mask[layer])
    
    def get_masks(self):
        return [module.get_masks() for module in self.h]
...
class GatedGPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=ProbingViaPromptingOutputs,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        if labels is not None:
            ...
            if self.eval_acc:
                # Token prediction 
                # Take out the tokens for prediction
                # There is only one token per example that has labels not equal to -100
                indices = labels != -100
                selected_labels = labels[indices]
                # print('selected_labels', selected_labels)
                num_tokens = lm_logits.shape[-1]
                correct_labels = selected_labels - num_tokens + self.num_labels # convert to 0-based to match with pred_labels
                # print('correct_labels', correct_labels)
                selected_logits = lm_logits[:,:-1,:][indices[:,1:]] # we need to shift one position backward for logits 
                pred_labels = selected_logits[:,-self.num_labels:].argmax(-1) # predict only from candidate tokens 
                # print('pred_labels', pred_labels)
                accuracy = (correct_labels == pred_labels).float()
            else:
                accuracy = None
        ...
        return ProbingViaPromptingOutputs(
            loss=loss,
            accuracy=accuracy,
            logits=lm_logits,
