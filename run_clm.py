...
from utils import convert_gate_to_mask
...
def main():
    ...
    if model_args.model_name_or_path:
        model = GatedGPT2LMHeadModel.from_pretrained(
    ...
    if model_args.head_mask_path == 'random':
        head_mask = torch.rand(12, 12)
        head_mask = convert_gate_to_mask(head_mask, 48)
        model.apply_masks(head_mask)
    elif model_args.head_mask_path != 'None':
    ...
    if model_args.head_mask_path != 'None':
        logger.info(f"Number of heads: {head_mask.sum()}")
